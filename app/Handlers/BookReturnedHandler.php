...
class BookReturnedHandler
{
    ...
    public function __invoke(ConsumerMessage $message, MessageConsumer $consumer): void
    {
        ...
        try {
            $span->setAttribute('user.member_card_uuid', $memberCardUUID ?? 'unknown');

            $dbSpan = $tracer->spanBuilder('database_fetch_user')
                ->startSpan();
            try {
            ...
            $mailSpan = $tracer->spanBuilder('send_return_borrow_email')
                ->startSpan();
            try {
                Mail::to($user->email)->send(new BookReturnedMail($notificationData));
